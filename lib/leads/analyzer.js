...
export function extractLinkedInSlug(companyUrl) {
  if (!companyUrl) return null
  const match = String(companyUrl).match(/linkedin\.com\/company\/([^/?#]+)/i)
  return match ? match[1] : null
}

export function buildLinkedInPeoplePageUrl(slug) {
  if (!slug) return null
  return `https://www.linkedin.com/company/${slug}/people/?facetGeoRegion=105117694`
}

export function buildLinkedInJobsPageUrl(slug) {
  if (!slug) return null
  return `https://www.linkedin.com/company/${slug}/jobs/`
}

export function buildLinkedInAboutPageUrl(slug) {
  if (!slug) return null
