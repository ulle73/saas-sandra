...
export function isSwedishPersonCandidate(person) {
  const location = normalizeForLookup(person?.location || '')
  if (!location) return false
  return SWEDEN_SIGNALS.some((term) => location.includes(term))
}
...
export async function findCompanyForLead(companyName, requestCounters, deadlineMs = null) {
  ...
  // If no dedicated Swedish profile, return the best match anyway.
  // We rely on person-level filtering (isSwedishPersonCandidate) in fetchCompanyPeople 
  // to ensure the final lead remains Swedish-focused.
  return ranked[0]
}
...
export async function fetchCompanyPeople(companyId, companyName, requestCounters, deadlineMs = null, titleStrategy = buildPersonTitleStrategy(DEFAULT_AI_PROFILE)) {
  ...
  for (let page = 1; page <= PEOPLE_MAX_PAGES; page += 1) {
    ...
    for (const row of rows) {
      ...
      if (STRICT_SWEDEN_ONLY && !isSwedishPersonCandidate(person)) {
        continue
      }
...
export function buildShortlistFromPeople(people, titleStrategy = buildPersonTitleStrategy(DEFAULT_AI_PROFILE)) {
  const shortlist = people
    ...
    .filter((item) => {
      ...
      if (STRICT_SWEDEN_ONLY && !isSwedishPersonCandidate(item.person)) return false
      return true
    })
...
export async function fetchCompanyAbout(companyUrlOrId, requestCounters, deadlineMs = null) {
  if (!companyUrlOrId) return null

  // Ensure we use the slug if it's a URL
  let target = companyUrlOrId
  if (String(target).includes('linkedin.com/company/')) {
    const match = String(target).match(/linkedin\.com\/company\/([^/?#]+)/i)
    if (match) target = match[1]
  }

  try {
    const payload = await requestRapidApi({
      host: PEOPLE_HOST, // Same host usually
      url: `https://${PEOPLE_HOST}/api/v1/company/profile`,
    ...
    const aboutText = payload?.data?.description || payload?.data?.about || payload?.about || payload?.description || null
    return aboutText
  } catch (error) {
    return null
  }
...
export async function fetchCompanyJobCount(companyId, requestCounters, deadlineMs = null) {
  if (!companyId) return null

  try {
    const payload = await requestRapidApi({
      host: PEOPLE_HOST,
      url: `https://${PEOPLE_HOST}/api/v1/company/jobs`,
      params: { company_id: companyId },
      step: 'company_job_count',
      counters: requestCounters,
      deadlineMs,
    })

    // Try to find the total results count in various possible locations
    const count = payload?.total 
      || payload?.data?.total_results 
      || payload?.data?.totalResults 
      || payload?.total_results 
      || payload?.totalResults 
      || (Array.isArray(payload?.data?.jobs) ? payload.data.jobs.length : null)
      || (Array.isArray(payload?.jobs) ? payload.jobs.length : null)
