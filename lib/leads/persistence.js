...
export async function persistLinkedInCompanyMatchForLead({ userId, candidate, companyInfo, aboutText = null }) {
...
export async function ensureDiscoverySchema() {
  const { error } = await supabase
    .from('lead_discovery_items')
    .select('company_name, status, source_url, contact_candidates, linkedin_company_id, linkedin_company_url, linkedin_people_search_hr_url, linkedin_people_search_ceo_url, linkedin_jobs_url, linkedin_people_url, linkedin_about_url, linkedin_about_text')
    .limit(1)
