...
export async function runCompanyPeopleFlow(userId, selectedCandidates, aiProfileInput = DEFAULT_AI_PROFILE) {
  ...
  for (let index = 0; index < selectedCandidates.length; index += 1) {
    ...
    const skipPeople = index >= peopleEnrichmentLimit
    await enrichLeadData(userId, candidate, { skipPeople })
  }

  return { generatedFiles: 0 /* deprecated count */, failedFiles: 0 }
}

export async function enrichLeadData(userId, candidate, options = {}) {
  ...
  try {
    console.log(`[ENRICH] Processing ${candidate.companyName}...`)
    
    let companyInfo = null
    if (options.manualLinkedInUrl) {
      const slug = extractLinkedInSlug(options.manualLinkedInUrl)
      companyInfo = {
        company_id: slug, // Use slug as ID if direct ID unknown
        name: candidate.companyName,
        company_url: options.manualLinkedInUrl
      }
    } else {
      companyInfo = await findCompanyForLead(candidate.companyName, requestCounters, deadlineMs)
    }

    if (!companyInfo?.company_id && !companyInfo?.company_url) {
      console.log(`- ${candidate.companyName}: no company info found`)
      return { success: false, reason: 'no_company_found' }
    }

    const { fetchAiProfilesByUserIdsLocal } = await import('./persistence.js')
    const aiProfiles = await fetchAiProfilesByUserIdsLocal([userId])
    const profile = aiProfiles.get(userId) || DEFAULT_AI_PROFILE
    const titleStrategy = buildPersonTitleStrategy(profile)
    ...
    if (options.skipPeople) {
      return { success: true, enriched: ['company', 'about'] }
    }
    ...
    return { success: true, enriched: ['company', 'about', 'people'], contactCount: contactCandidates.length }
  } catch (error) {
    console.error(`- ${candidate.companyName} enrichment failed:`, error.message)
